...
class Exercise:
    ...
    def repetitions_counter(self, img, counter):
        cv2.rectangle(img, (0, 0), (1400, 73), (245, 117, 16), -1)

        # Rep data
        cv2.putText(img, '', (15, 12),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 1, cv2.LINE_AA)
        ans=""
        if str(counter) in "0":
            ans="Start Your Exercise"
        elif str(counter) in "148":
            ans="Make your hand straight"
        else:
            ans="Yes, Correct Posture, Keep Doing"
        cv2.putText(img, ans,
                    (10, 60),
